...
export async function sendTicketEmail(ticketId: string) {
    ...
    if (ticket.slot) {
        ...
        // Find the specific event dates using the exact location name match against the title
        const locationName = ticket.product?.location?.name;
        if (locationName) {
            eventTitle = locationName; // Use the precise location name as the Event Title

            const { data: events } = await supabaseAdmin
                .from('app_events')
                .select('title, dates');

            // Strict exact match against the title to find dates
